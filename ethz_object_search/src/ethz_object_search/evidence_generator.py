...
from ethz_object_search.data_structure import Box
...
class LocationEvidenceGenerator(EvidenceGenerator):
    ...
    def getEvidence(self, dataStrs):
        ...
        classes = dataStrs[0].getClassNames()

        # declare evidence as dictionary where keys are the object pairs
        # and initialize evidence entries to hold numpy arrays
        #TODO: not recommended for numpy!
        evidence = {(classPair, np.zeros((0, 2))) for classPair in itertools.product(classes, repeat=2)}
